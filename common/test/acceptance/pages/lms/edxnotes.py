...
class EdxNoteHighlight(NoteChild):
    ...
    TEXTAREA_SELECTOR = ".annotator-item textarea"
    ...
    def text(self):
        ...
        """
        self.show().edit()
        element = self.q(css=self._bounded_selector(self.TEXTAREA_SELECTOR))
        if element:
            text = element.attrs("value")[0]
        else:
            text = None
        self.cancel()
        return text
    ...
    def text(self, value):
        ...
        """
        self.q(css=self._bounded_selector(self.TEXTAREA_SELECTOR)).first.fill(value)
